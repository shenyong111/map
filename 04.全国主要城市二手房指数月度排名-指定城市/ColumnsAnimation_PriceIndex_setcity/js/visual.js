    ...
    timer.transition().duration(intervalTime*2000).delay(intervalTime*1000*isFirst).ease(d3.easeLinear)
        ...
        .tween("text", function(d){
            ...
            return function (t) {
                ...
                let year = timeCounter.value.toString().substr(0,4);
                let month = timeCounter.value.toString().substr(4,2)
...
function refresh(data) {
    ...
    barEnter.append("text")
        ...
        .attr("y", 0)
        .attr("class", function (d) {
                return  "label leftText";
        })
        .attr("x", -5) //位置
        .attr("y", barConfig.yValue) //x位置
        .attr("text-anchor", "end")
        .text(d => d.name);
    var label = g.selectAll(".bar .label");
    label.attr("class",function(d){
        if(d.name == app.mainCity){
            return  "label mainCity ";
        }else{
            return  "label leftText";
        }
    });
...
function queryData(yearmonth,callback){
    var city_ids = app.cityList.map(function(el,index){return el.cityId}).join(',');
    if(app.mainCityId&&city_ids) city_ids +=  ','+app.mainCityId;
    $.ajax({
...
function queryCallback(response){
    ...
    if(app.mainCityId){
        for(var j = 0;j<reankedData.length;j++){
            if(app.mainCity == reankedData[j].name){//主城市在排序后的结果集中的位置
               if(j>=config.MaxNumber-1){
                   topReankedData[config.MaxNumber-1] = reankedData[j];
               }
                break;
            }
        }
